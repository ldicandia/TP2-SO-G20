...
orderedListADT new_ordered_list(t_compare cmp) {
	orderedListADT aux = allocMemory(sizeof(*aux));
	if (aux == NULL) {
		return NULL;
	}
	aux->size  = 0;
...
static t_ordered_list add_ordered_list_rec(t_ordered_list list, elem_type elem,
										   t_compare cmp, int *flag) {
	int c;
	if (list == NULL || (c = cmp(elem, list->head)) < 0) {
		t_ordered_list aux = allocMemory(sizeof(*aux));
		if (aux == NULL) {
			*flag = 0;
			return list;
		}
...
int add_ordered_list(orderedListADT list, elem_type elem) {
...
static t_ordered_list delete_rec(t_ordered_list list, elem_type elem,
								 t_compare cmp, int *flag) {
	int c;
