...
typedef struct Process { 
	...
	char **argv;
	char *name; // nombre del proceso, para q sea mas facil de identificar q x su pid
	uint8_t unkillable;
	uint8_t priority;
...
void initProcess(Process *process, uint16_t pid, uint16_t parentPid, char **args, char *name, uint8_t priority, int16_t fileDescriptors[], uint8_t unkillable);
