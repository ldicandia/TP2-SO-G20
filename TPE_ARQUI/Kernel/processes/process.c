...
pid_t wait(pid_t pid, int64_t *ret) {
	...
	if ((pcb_to_wait == NULL) || (pcb_to_wait->status == FREE) ||
		(pcb_to_wait->waiting_me != NULL) || (pid == get_pid())) {
		return -1;
	}
	...
	if (!((pcb_to_wait->status ==
		   ZOMBIE))) { // This could happen if it was killed
		return -1;
	}
...
int64_t new_process(main_function rip, priority_t priority, uint8_t killable,
	...
	uint64_t rsp_malloc = (uint64_t) allocMemory(STACK_SIZE);
	...
	// 		freeMemory((void *) pcb_array[pid].argv[i]);
	// 	}
	// 	freeMemory((void *) pcb_array[pid].argv);
	// 	pcb_array[pid].status = FREE;
	// 	return -1;
	// }
...
static int64_t set_free_pcb(PCB *process) {
	...
	freeMemory((void *) process->lowest_stack_address);
	if (process->argv == NULL) {
		process->status = FREE;
		return 0;
	}
	for (uint64_t i = 0; i < process->argc; i++) {
		freeMemory((void *) process->argv[i]);
	}
	freeMemory((void *) process->argv);
	process->status = FREE;
...
void get_process_info(PCB *pcb, process_info *process) {
	process->name = new_str_copy(
		pcb->argv != NULL ? pcb->argv[0] :
							NULL); // If malloc fails, we print it as "No name"
...
process_info_list *ps() {
	process_info_list *process_list = allocMemory(sizeof(process_info_list));
	if (process_list == NULL) {
		return NULL;
	}
	process_list->amount_of_processes = amount_of_processes;
	process_info *processes =
		allocMemory(amount_of_processes * sizeof(process_info));
	if (processes == NULL) {
		freeMemory((void *) process_list);
		return NULL;
	}

	for (int i = 0, found = 0; i < PCB_AMOUNT && found < amount_of_processes;
		 i++) {
...
void free_ps(process_info_list *ps) {
	...
	for (int i = 0; i < ps->amount_of_processes; i++) {
		if (ps->processes[i].name) {
			freeMemory(ps->processes[i].name);
		}
	}
	freeMemory(ps->processes);
	freeMemory(ps);
}
...
// 	if (shell_pcb == NULL ||
// 		((foreground_process = shell_pcb->waiting_for) == NULL)) {
// 		return;
// 	}
// 	// first, we wait on the right side of the pipe.
// 	PCB *other_process_in_pipe = NULL;
// 	if (foreground_process->fds[STDIN] > MAX_COMMON_FD) {
// 		other_process_in_pipe =
// 			get_pcb(pipe_get_pid(foreground_process->fds[STDIN] - 3, WRITER));
// 	}
