...
static MemoryBlock *createMemoryBlock(void *ptr, uint8_t exp,
									  MemoryBlock *next) {
...
void freeMemory(void *ptr) {
	...
	MemoryManagerADT mm = getMemoryManager();
	MemoryBlock *block =
		(MemoryBlock *) ((uint8_t *) ptr - sizeof(MemoryBlock));
	if (block->used == FREE)
		return;
	block->used = FREE;

	uint64_t relPos =
		(uint64_t) ((uint8_t *) block - (uint8_t *) mm->firstAddress);
	MemoryBlock *buddy = (MemoryBlock *) ((uint8_t *) mm->firstAddress +
										  (relPos ^ (1UL << block->exp)));
	while (buddy->used == FREE && buddy->exp == block->exp &&
		   block->exp < mm->maxExp) {
		block  = merge(mm, block, buddy);
		relPos = (uint64_t) ((uint8_t *) block - (uint8_t *) mm->firstAddress);
		buddy  = (MemoryBlock *) ((uint8_t *) mm->firstAddress +
								  (relPos ^ (1UL << block->exp)));
	}
	mm->blocks[block->exp - MIN_EXP] = createMemoryBlock(
		(void *) block, block->exp, mm->blocks[block->exp - MIN_EXP]);
}
