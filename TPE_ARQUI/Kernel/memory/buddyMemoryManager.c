...
/*División: Los bloques grandes se dividen en pares de bloques iguales (buddies).
Asignación: Se asigna el bloque del tamaño adecuado (o uno de los buddies si es q se dividió).
Fusión: Cuando dos bloques buddy contiguos están libres, se fusionan para formar un bloque más grande. */
...
typedef struct MemoryManagerCDT				
{
	...
	BuddyBlock *blocks[LEVELS]; // Array de listas doblemente enlazadas de bloques libres (tantos niveles)
...
static uint8_t next_exp(uint64_t n) {//encuentra el menor exponenta tal que 2^exp sea mayor o igual a n
    uint8_t exp = 0;
    while ((1UL << exp) < n) {//1UL
        exp++;
    }
...
static BuddyBlock *createBuddyBlock(void *address, uint8_t exp, BuddyBlock *next);
	static BuddyBlock *removeBuddyBlock(MemoryManagerADT memoryManager, BuddyBlock *block);
	static BuddyBlock *merge(MemoryManagerADT memoryManager, BuddyBlock *block, BuddyBlock *buddy);
	static void split(MemoryManagerADT memoryManager, uint8_t index);
	static void insertFreeBlock(uint8_t index, BuddyBlock *block, MemoryManagerADT memoryManager);
...
MemoryManagerADT createMemoryManager(void *const restrict memoryForMemoryManager,
	void *const restrict managedMemory, uint64_t memAmount){
		...
		MemoryManagerADT memoryManager = (MemoryManagerADT) memoryForMemoryManager;
		 ...
		 BuddyBlock *initialBlock = createBuddyBlock(managedMemory, memoryManager->maxExp , NULL);//no tiene previo
		 insertFreeBlock(memoryManager->maxExp - MIN_BUDDY_EXP, initialBlock, memoryManager); // Insertar en la lista correcta
    ...
    // Si no hay bloques libres en el nivel, buscar el más cercano superior y dividir
    int closestIdx = -1;
    ...
    BuddyBlock *block = removeBuddyBlock(memoryManager, memoryManager->blocks[idxToAlloc]);
    block->used = USED;
    block->prev = NULL;
    block->next = NULL;
    ...
    // Calcular la posición relativa del bloque respecto al inicio de la memoria administrada
    uint64_t relativePosition = (uint64_t)((uint8_t *)block - (uint8_t *)memoryManager->Address);
    ...
    while (block->exp < memoryManager->maxExp) {
        ...
        buddy = (BuddyBlock *)((uint8_t *)memoryManager->Address + buddyOffset);
        ...
        if (buddy->used == FREE && buddy->exp == block->exp) {
            ...
            relativePosition = (uint64_t)((uint8_t *)block - (uint8_t *)memoryManager->Address);
        } else {
            break;
        }
    }
	...
	static BuddyBlock *merge(MemoryManagerADT memoryManager, BuddyBlock *block, BuddyBlock *buddy){
		removeBuddyBlock(memoryManager, buddy);  //el buddy se elimina, xq se fusionan en uno

		BuddyBlock *leftBlock = (BuddyBlock *)((uintptr_t)block < (uintptr_t)buddy ? block : buddy);
		leftBlock->exp++;
    ...
    uint64_t blockSize = 1UL << (index + MIN_BUDDY_EXP); // Tamaño del bloque a dividir
