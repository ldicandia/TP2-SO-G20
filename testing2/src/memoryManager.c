...
MemoryManagerADT createMemoryManager(void *const restrict memoryForMemoryManager, void *const restrict managedMemory,
	...
	char *startOfUserMemory = (char *) managedMemory + sizeof(MemoryBlock) * MAX_BLOCKS;
...
static void addBlock(MemoryManagerADT memoryManager, void *address, uint64_t size) {
...
static bool removeBlock(MemoryManagerADT memoryManager, void *address) {
	...
	for (int i = 0; i < memoryManager->blockCount; i++) {
		if (memoryManager->blocks[i].address == address) {
			...
			memoryManager->blocks[i] = memoryManager->blocks[memoryManager->blockCount - 1];
			memoryManager->blockCount--;
			return true;
		}
	}
